/* evpdecrypt.c - function for decrypting via EVP library */
...
/* OpenSSl's evp decryption routines.
...
{
    /*This will hold the updated length after EVP_DecryptFinal_ex*/
    int evpLengthUpdate = 0;

    if (!EVP_DecryptUpdate(ctx, decryptedBufferLcl, evpOutputLength, encryptedBufferLcl, evpInputLength)) {
    ...
    if (!EVP_DecryptFinal_ex(ctx, decryptedBufferLcl + *evpOutputLength, &evpLengthUpdate)) {
