...
int writePass(FILE* dbFile)
{
    ...
    if (toggle.firstRun == 1) {
        ...
        fwrite(encryptedBuffer, 1, outlen * sizeof(unsigned char), dbFile);
...
int sealEnvelope(const char* tmpFileToUse)
{
    ...
    fwrite(gMac, sizeof(unsigned char), SHA512_DIGEST_LENGTH, EVP1DataFileTmp);
    ...
    /*Write the salt*/
    fwrite(evp2Salt, sizeof(unsigned char), EVP2_SALT_SIZE, dbFile);
    ...
    /*Write the salt*/
    fwrite(evp1Salt, sizeof(unsigned char), EVP1_SALT_SIZE, dbFile);
...
int openEnvelope()
{
    ...
    fwrite(tmpBuffer, sizeof(unsigned char), fileSize - SHA512_DIGEST_LENGTH, EVP1DataFileTmp);
    if(!returnVal == fileSize - SHA512_DIGEST_LENGTH/sizeof(unsigned char));
            {
				printf("fwrite failed\n");
				return 1;
